...
float g_width, g_height, g_left, g_right, g_top, g_bottom;
...
int w2px(float x){
   float imgX = ((2 * x) / (g_right - g_left)) + ((-g_right - g_left) / (g_right - g_left));
   return ((g_width * imgX) / 2) + ((g_width - 1) / 2);
}

int w2py(float y){
   float imgY = (2 * y / (g_top - g_bottom)) + ((-g_top - g_bottom) / (g_top - g_bottom));
   return ((g_height * imgY) / 2) + ((g_height - 1) / 2);
}
...
int main(int argc, char **argv)
{
   ...
   if(g_width > g_height){
      g_left = -g_width/g_height;
      g_right = g_width/g_height;
      g_top = 1;
      g_bottom = -1;
   } else {
      g_left = -1;
      g_right = 1;
      g_top = g_height/g_width;
      g_bottom = -g_height/g_width;
   }
   ...
   for (int i = 0; i < triBuf.size(); i++)
   {
      ...
      float x0 = posBuf[3*i0];
      float y0 = posBuf[3*i0+1];

      int px = w2px(x0);
      int py = w2py(y0);

      image->setPixel(px, py, 255, 255, 255);
   }
